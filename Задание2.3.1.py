...
dict_count_area  = dict_count_area.items()
dict_count_area = {x[0]: str(f'{x[1]*100:,.2f}%').replace('.',',') for x in dict_count_area}

image = "graph.png"
env = Environment(loader=FileSystemLoader('.'))
template = env.get_template("pdf_template.html")
year = ["Год", "Средняя зарплата", "Средняя зарплата - Программист", "Количество вакансий", "Количество вакансий - Программист"]
city = ["Город", "Уровень зарплат", "Город", "Доля вакансий"]
pdf_template = template.render({'dict_salary': dict_salary,
                                'dict_count': dict_count, 'dict_salary_vac': dict_salary_vac,
                                'dict_count_vac': dict_count_vac, 'dict_salary_area': dict_salary_area,
                                'dict_count_area': dict_count_area, 'year': year, 'city': city, 'image': image})
